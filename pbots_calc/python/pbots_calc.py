...
try:
    ...
    print(error)
    print ("ERROR: Could not locate pbots_calc. Please ensure your enviroment library load path is set properly.")
    sys.exit(1)
...
def calc(hands, board, dead, iters):
    ...
    err = pcalc.calc(ctypes.c_char_p(hands.encode('utf-8')), ctypes.c_char_p(board.encode('utf-8')), ctypes.c_char_p(dead.encode('utf-8')), ctypes.c_int(iters), res)
    if err > 0:
